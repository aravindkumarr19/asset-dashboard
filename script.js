...
// from the same cleaned tracker rows. No sample data is used.
// ============================================================
...
const elements = {
  ...
  totalEligibility: document.querySelector("#totalEligibility"),
  totalCrores: document.querySelector("#totalCrores"),
  totalPH: document.querySelector("#totalPH"),
  liveThisWeek: document.querySelector("#liveThisWeek"),
  ...
  slackReport: document.querySelector("#slackReport"),
  copyWeeklyReportButton: document.querySelector("#copyWeeklyReportButton"),
  copySlackReportButton: document.querySelector("#copySlackReportButton"),
...
async function initDashboard() {
  ...
  elements.copySlackReportButton.addEventListener("click", () => copyReport(elements.slackReport, elements.copySlackReportButton, "Copy Slack Report"));
...
function parseCsv(csvText) {
  ...
  return rows.map((row) => {
    ...
    headers.forEach((header, index) => {
      const dashboardColumn = findDashboardColumn(header);
      if (dashboardColumn) {
...
function renderSummaryCards() {
  const totalEligibilityNumber = sumBy(visibleRows, "Eligibility");
  ...
  elements.totalEligibility.textContent = formatCurrencyCrores(totalEligibilityNumber);
  elements.totalCrores.textContent = formatCroresCurrency(totalCroresNumber);
  elements.totalPH.textContent = productionHouses.length.toLocaleString("en-IN");
  elements.liveThisWeek.textContent = getUpcomingRows(visibleRows).length.toLocaleString("en-IN");
...
function renderWeeklyReport() {
  elements.weeklyReport.value = buildReportText("Weekly Asset Status Report", true);
}

function renderSlackReport() {
  elements.slackReport.value = buildReportText("Asset Status Tracker Summary", false);
}

function buildReportText(title, includeLiveList) {
  const totalCroresNumber = sumBy(visibleRows, "Eligibility in Crores");
  const alertData = getAlertData(visibleRows);
  const upcomingAssets = getUpcomingRows(visibleRows);
  const dataQualityCount = countMissingFields(visibleRows) + alertData.invalidLiveWeek.length;
  const assetList = upcomingAssets.length
    ? upcomingAssets.map((row) => `- ${clean(row["Asset Name"]) || "Unnamed asset"} | ${clean(row["Movie Name"]) || "Movie missing"} | Live Date: ${clean(row["Live Date"]) || "Missing"}`).join("\n")
    : "- No assets with live dates in the next 7 days.";

  const lines = [
    `*${title}*`,
    "",
    `Total assets: ${visibleRows.length}`,
    `Total eligibility: ${formatCurrencyCrores(sumBy(visibleRows, "Eligibility"))}`,
    `Total eligibility in crores: ${formatCroresCurrency(totalCroresNumber)}`,
    `Production houses: ${uniqueValues(visibleRows, "PH Name").length}`,
    `Data quality alerts: ${dataQualityCount}`,
    ""
  ];

  if (includeLiveList) {
    lines.push("*Assets going live:*", assetList, "");
  }

  lines.push(
    "*Alerts:*",
    `- Missing eligibility: ${alertData.missingEligibility.length}`,
    `- Missing eligibility in crores: ${alertData.missingCrores.length}`,
    `- Missing PH name: ${alertData.missingPH.length}`,
    `- Missing live date: ${alertData.missingLiveDate.length}`,
    `- Duplicate asset names: ${alertData.duplicateAssets.length}`,
    `- Invalid live week values: ${alertData.invalidLiveWeek.length}`,
    "",
    `Google Sheet: ${SHEET_CONFIG.sheetLink}`,
    "",
    "Please review the tracker and resolve open data quality items before circulation."
  );
...
function showFetchError() {
  ...
  if (allRows.length === 0) {
    ...
    elements.slackReport.value = "Unable to fetch Google Sheet data";
  }
}
